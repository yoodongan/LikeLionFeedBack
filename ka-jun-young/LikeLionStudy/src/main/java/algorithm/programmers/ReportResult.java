   ...
   */
    public static int[] solution(String[] id_list, String[] report, int k) {
        ...
        for (String reportInfo : report) {
            ...
            if (!userReportInfoMap.get(reporter).contains(reported)) {
                // 중복 방지를 위해 신고한 유저의 신고 목록에 reported 가 없다면 저장

                String reportId = userReportInfoMap.get(reporter) + " " + reported;
                // 신고를 한 유저의 신고 목록을 받아와서 reported 추가

                userReportInfoMap.replace(reporter, reportId.trim());
            ...
            if (reportedCount >= k) {
                for (Map.Entry<String, String> entry : userReportInfoMap.entrySet()) {
                    if (entry.getValue().contains(userId)) {
                        int userMailCount = userReportedCountMap.get(entry.getKey());
                        userReportedCountMap.replace(entry.getKey(), userMailCount + 1);
                    }
                }
            }
        }
